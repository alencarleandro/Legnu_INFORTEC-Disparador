...
public class MaskedTextField extends TextField {
	...
	private void init() {
		...
		// Add a listener to the plain text property so that binding will properly
		// update the formatting as well
	...
	/**
	 * Returns the character to use in place of characters that are not present in
	 * the value, ie the user must fill them in.
	 */
	...
	/**
	 * Build internal mask from input mask using AbstractFactory to add the right
	 * MaskCharacter.
	 */
	...
	private boolean isLiteral(char c) {
		return (c != MASK_ANYTHING && c != MASK_CHARACTER && c != MASK_ESCAPE && c != MASK_NUMBER
				&& c != MASK_CHAR_OR_NUM && c != MASK_HEXADECIMAL && c != MASK_LOWER_CHARACTER
